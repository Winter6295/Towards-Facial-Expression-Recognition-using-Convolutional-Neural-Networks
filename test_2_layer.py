...
each_pixel_mean = train_images.mean(axis=0)
each_pixel_std = np.std(train_images, axis=0)
test_images = np.divide(np.subtract(test_images,each_pixel_mean), each_pixel_std)
predicted_labels = np.zeros(test_images.shape[0])


with tf.Session() as sess:
	saver = tf.train.import_meta_graph('my_model_version_9.meta')
	saver.restore(sess, 'my_model_version_9')
	...
	for i in range(0,test_images.shape[0]//batch_size):
		...
		predicted_labels[i*batch_size : (i+1)*batch_size] = sess.run(predict, feed_dict={"x:0":test_images[i*batch_size : (i+1)*batch_size], "keep_prob:0":1.0})
